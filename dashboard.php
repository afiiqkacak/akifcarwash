...
FROM queue_service NATURAL JOIN service
...
$sql2 = "SELECT category_of_car, COUNT(category_of_car) AS frequency 
FROM `queue` INNER JOIN car INNER JOIN car_category
WHERE car.car_id=queue.car_id AND car.car_category_id=car_category.car_category_id
GROUP BY category_of_car
ORDER BY frequency DESC";
...
WHERE queue_service.queue_id=queue.queue_id AND service.service_id=queue_service.service_id AND date=DATE_FORMAT(SYSDATE(),"%Y-%c-%d")
GROUP BY service_type
ORDER BY total DESC';
$result3 = mysqli_query($connect,$sql3) or die (mysqli_error($connect));

$sql4 = "SELECT category_of_car AS kereta, DATE_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(TIMEDIFF(completed_time,arrival_time)))),'%H:%i') AS avgtime
FROM queue INNER JOIN car INNER JOIN car_category
WHERE queue.car_id=car.car_id AND car.car_category_id=car_category.car_category_id AND completed_time != '00:00:00'
GROUP BY kereta";
...
FROM `queue`
WHERE date=DATE_FORMAT(SYSDATE(),'%Y-%c-%d')
GROUP BY time";
...
$tar= date("Y-m-d");?>
			...
			<ul class="nav nav-tabs">
				<li class="nav-item">
					<a class="nav-link active" href="dashboard.php">Dashboard <i class="fa fa-tachometer" aria-hidden="true"></i></a>
				</li>
				<li class="nav-item">
